...
The **secure profile** layers on zero-trust workload identity (SPIRE mTLS), Keycloak OAuth2/JWT authentication, and an Istio ambient mesh — enforced by an AuthBridge sidecar. It requires additional cluster components (SPIRE, Keycloak, Istio ambient) and a few `Kagenti` spec settings, and each agent namespace must be labeled `kagenti-enabled=true`. To turn it on, see [Enable the Secure Profile](../../../administer/secure_profile/enable_secure_profile.mdx); for the architecture, see [Security Architecture](./security_architecture.mdx).

:::info
A `Succeeded` CSV plus a reconciled `Kagenti` resource means the controllers are running. Agents and tools themselves only appear once you create the workloads and `AgentRuntime` resources — see [Deploy an Agent with AgentRuntime](../../deploy_agent_with_agentruntime.mdx).
:::
