...
Under the core profile there is no authentication or transport security — every step below is a plain in-cluster HTTP call. For the secured version (Bearer-token requests, SPIRE mTLS, an AuthBridge sidecar per agent), continue to [Demo with Secure Profile](./demo_with_secure_profile.mdx) after this guide.

## Prerequisites

- Kagenti Operator is installed and its `Kagenti` operand is reconciled — see [Installation](./components/kagenti/install.mdx).
...
### Find your model endpoint and name \{#find-your-model-endpoint-and-name\}
...
| Variable | Value |
|----------|-------|
| `LLM_API_BASE` | `http://qwen36-27b-gguf-predictor.models.svc.cluster.local/v1` |
...
## Step 1: Deploy the MCP tool server \{#step-1-deploy-the-mcp-tool-server\}
...
## Step 2: Deploy the agent \{#step-2-deploy-the-agent\}
  ...
  template:
    ...
    spec:
      ...
      - name: agent
        ...
        - name: LLM_API_BASE
          value: http://qwen36-27b-gguf-predictor.models.svc.cluster.local/v1 # [!code callout]
        - name: LLM_API_KEY
          value: dummy
        - name: LLM_MODEL
...
2. The agent image, mirrored to `docker.io/alaudadockerhub`. Use `v0.1.0` or later — these read `HOST`/`PORT` from the environment, which is required under the [secure profile](../administer/secure_profile/enable_secure_profile.mdx) (the AuthBridge sidecar remaps the app port).
...
## Step 4: Send a query (end-to-end test) \{#step-4-send-a-query-end-to-end-test\}
...
1. [Install the Secure-Profile Dependencies](../administer/secure_profile/install_secure_dependencies.mdx) — SPIRE, Keycloak, Istio ambient.
2. [Enable the Secure Profile](../administer/secure_profile/enable_secure_profile.mdx) — flip the operand feature gates.
3. [Demo with Secure Profile](./demo_with_secure_profile.mdx) — re-run the same weather scenario with an AuthBridge sidecar on the agent, a SPIRE SVID per workload, and Bearer-token-authenticated requests.
4. [Demo Advanced: AuthBridge on the Tool with Token Exchange](./demo_advanced_secure_profile.mdx) — add AuthBridge to the tool too, with RFC 8693 token exchange between agent and tool.
