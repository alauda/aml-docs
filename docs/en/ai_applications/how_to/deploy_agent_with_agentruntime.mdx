...
2. The agent image, mirrored to `docker.io/alaudadockerhub`. Use `v0.1.0` or later — these read `HOST`/`PORT` from the environment, which is required under the [secure profile](../../administer/how_to/enable_secure_profile.mdx) (the AuthBridge sidecar remaps the app port).
...
1. [Install the Secure-Profile Dependencies](../../administer/how_to/install_secure_dependencies.mdx) — SPIRE, Keycloak, Istio ambient.
2. [Enable the Secure Profile](../../administer/how_to/enable_secure_profile.mdx) — flip the operand feature gates.
