...
1. The **secure-profile dependencies are installed** — cert-manager, SPIRE, Keycloak, Istio ambient mesh. See [Install the Secure-Profile Dependencies](../administer/secure_profile/install_secure_dependencies.mdx).
2. The **secure profile is enabled** on the `Kagenti` operand (`spec.featureGates.globalEnabled: true`, `spec.authbridgeConfig.enabled: true`, `spec.keycloak.publicUrl` set). See [Enable the Secure Profile](../administer/secure_profile/enable_secure_profile.mdx).
3. An **InferenceService that serves an OpenAI-compatible chat API** is reachable in-cluster. This guide uses `qwen36-27b-gguf` in `models`; substitute your own everywhere you see those names — see [the model-endpoint recipe in the quick start](./deploy_agent_with_agentruntime.mdx#find-your-model-endpoint-and-name) for how to resolve `LLM_API_BASE`, `LLM_MODEL`, and `LLM_API_KEY`.
...
| `ImagePullBackOff` on an AuthBridge image | The `defaults.images` on the `Kagenti` operand points at a registry the cluster can't pull. Relocate the AuthBridge sidecar images (see [Enable the Secure Profile](../administer/secure_profile/enable_secure_profile.mdx)) into a registry your cluster can reach. |
