...
Follow the instructions in [Create Workbench](../../develop/workbench/how_to/create_workbench.mdx) to create a new Workbench instance. When creating the Workbench, make sure to select the image `odh-workbench-jupyter-pytorch-llmcompressor-cuda-py312-ubi9`, which includes the required LLM Compressor environment. Model compression is currently supported only within **JupyterLab**.
...
Refer to [Upload Models Using Notebook](../../develop/how_to/upload_models_using_notebook.mdx) for detailed steps on creating a model repository and uploading your model files. The example notebooks in this guide use the [TinyLlama-1.1B-Chat-v1.0](https://huggingface.co/TinyLlama/TinyLlama-1.1B-Chat-v1.0) model.
...
Once compression is complete, upload the compressed model back to the model repository. See [Upload Models Using Notebook](../../develop/how_to/upload_models_using_notebook.mdx) for detailed steps on uploading model files to the model repository.
...
Follow the instructions in [create inference service](../inference_service/functions/inference_service.mdx#create-inference-service) to complete this step.
