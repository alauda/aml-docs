...
If you want to manage reusable project-level S3, URI, or OCI endpoints in the UI, see [Using Connections](../../develop/connections/using_connections.mdx).
...
2. `aml.cpaas.io/runtime-type: vllm` specifies the code runtime type. For more information about custom inference runtimes, see [Extend Inference Runtimes](../inference_service/guides/custom_inference_runtime.mdx).
...
For detailed instructions on packaging and deploying models using OCI containers, see [Using KServe Modelcar for Model Storage](../inference_service/guides/using_modelcar.mdx).
...
- The workbench is attached to the persistent volume (PVC).
  ...
  For instructions on creating a workbench and attaching a PVC, see [Create Workbench](../../develop/workbench/create_workbench.mdx).
