...
- [Authenticating Consumers](../../gateway/identity_authentication) — verify SSO tokens or API keys and propagate caller identity.
- [Configuring Token Quotas](../../gateway/token_rate_limiting) — enforce per-user, per-department, and per-tier token budgets.
- [Metering Token Usage](../../gateway/usage_metering) — report consumption per tenant and feed chargeback.
- [Routing to LLM Providers](../../gateway/external_provider_routing) — front external providers with credential injection and failover.
- [Charging Back Token Usage](../../gateway/cost_management_chargeback) — price token usage into per-namespace bills with Cost Management.

:::warning Create the gateway in a dedicated namespace
