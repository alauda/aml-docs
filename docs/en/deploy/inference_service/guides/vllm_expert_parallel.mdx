...
:::warning
This page focuses on the single-node configuration pattern. For the Alauda AI
leader/worker deployment workflow, see
[Deploy a Multi-Node vLLM Service with llm-d](./llm-d-multi-node.mdx), then add
the EP flags supported by your model and runtime image. Also review the official
vLLM guide for the EP-specific communication topology.
:::
