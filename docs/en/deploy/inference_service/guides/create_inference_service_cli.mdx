...
Create a YAML file named `qwen-2-vllm.yaml` with the following content:
...
qwen-2   http://qwen-2.<your-namespace>.example.com True                                                                  2m
```

The revision columns stay empty because this service runs in `Standard` deployment mode,
which creates a plain Kubernetes Deployment rather than a Knative revision.
