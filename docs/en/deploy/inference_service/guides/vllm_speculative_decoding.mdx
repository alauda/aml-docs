...
| Service stays NotReady during rollout, or Knative marks the revision NotReady with a progress-deadline timeout | Cold start with a draft artifact is slower than without — torch.compile of both backbone and EAGLE head + engine profiling can push it past the readiness deadline. Our EAGLE-3 cold start on A30 + Llama-3.1-8B was ~5 min | In `Standard` mode, raise the predictor readiness probe so it tolerates a 5-minute start, for example `initialDelaySeconds: 60` with `periodSeconds: 10` and `failureThreshold: 30`. In `Knative` mode, set the `serving.knative.dev/progress-deadline` annotation to `1800s` |
