...
- **Alauda AI Workbench v0.2.0** is installed through the Workbench OLM Operator. Create the `Workbench` CR in the `kubeflow` namespace. Use Alauda AI Operator (`aml-operator`) v2.8.1, which automatically corrects the v0.2.0 Workbench-CR namespace issue.
...
| **Alauda AI Workbench** | Workbench OLM Operator | Install Workbench v0.2.0 and create its `Workbench` custom resource in `kubeflow`; use `aml-operator` v2.8.1 to automatically correct the namespace issue. |
