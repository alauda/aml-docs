...
**Alauda Build of NPU Operator** is refactored from a cluster plugin to an operator-managed component so it can be managed by the Alauda AI Operator. The delivered component is updated to a newer version aligned with the community release, including component bug fixes.
