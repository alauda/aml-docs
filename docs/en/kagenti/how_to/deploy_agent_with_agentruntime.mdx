...
With the secure profile on, an **unauthenticated** request to the agent is rejected:

```json
{"error":"auth.unauthorized","message":"missing Authorization header","plugin":"jwt-validation"}
```

Send a valid Keycloak access token as `Authorization: Bearer <token>`. Obtain one with a `client_credentials` grant using the workload's registered client (`team1/weather-agent`; the client id and secret are in the `kagenti-keycloak-client-credentials-*` secret), then repeat the [Step 4](#step-4-send-a-query-end-to-end-test) query with the `Authorization` header added.
:::
