...
The MCP (Model Context Protocol) server provides the `get_weather` tool the agent calls. Deploy it and expose it with a Service. **This guide follows the upstream weather-agent demo pattern**: the tool workload runs as a plain `Deployment` with **no `AgentRuntime` and no AuthBridge sidecar** — a Kagenti `AgentRuntime` is only created for the agent side. The security posture (see [Security architecture](../security_architecture.mdx)) is inbound-only on the agent: the agent's AuthBridge validates the client's JWT, then the agent → tool call is a plain in-cluster HTTP hop, and the tool reaches its public data source (`geocoding-api.open-meteo.com`) directly.

:::info
Injection into tool workloads is off by default: the Kagenti operand's `spec.featureGates.injectTools` defaults to `false`, so tagging a Deployment with `kagenti.io/type: tool` does **not** on its own inject a sidecar. Flip `injectTools: true` on the `Kagenti` operand only when you need inbound JWT validation on the tool itself (or outbound token exchange from the tool). Injecting an AuthBridge sidecar into the tool would also route the tool's outbound calls through its HTTP forward proxy, which is HTTP-only and blocks external HTTPS with a `CONNECT` `405`; see [Allowing outbound traffic in `proxy-sidecar` mode](#allowing-outbound-traffic-in-proxy-sidecar-mode) for the `NO_PROXY` escape hatch when you do need injection on the tool.
:::
...
`READY 1/1` confirms the tool is running as a single container — the agent side is the only workload with an injected AuthBridge sidecar (`READY 2/2`, added in the next step).
...
```bash
# AgentRuntime status — only the agent side has one in this pattern
...
# Pods: agent 2/2 (sidecar injected), tool 1/1 (no sidecar)
...
The rest of this section applies only when you deliberately inject an AuthBridge sidecar into a workload that also needs external HTTPS (typically by flipping `injectTools: true` on the `Kagenti` operand, or by adding `kagenti.io/authbridge-inject: "true"` to an individual Deployment). AuthBridge's HTTP forward proxy is **HTTP-only** — it responds `405 Method Not Allowed` to `CONNECT`, so an injected app making an outbound HTTPS request through the sidecar hits `Tunnel connection failed: 405`. This is by design: the outbound `token-exchange` plugin needs to read and modify request headers, which is not possible through an opaque TLS tunnel.
...
| Agent answers without live data | Verify `MCP_URL` points at the `weather-tool-mcp` Service and the tool pod is `Running`: `kubectl logs -l app.kubernetes.io/name=weather-tool -n team1`. |
