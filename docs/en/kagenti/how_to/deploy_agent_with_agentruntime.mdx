...
:::warning
The Envoy config template in this release wires transport security through file-based SDS but the operator does not yet render matching SPIRE-issued SVIDs into Envoy. `envoy-sidecar` therefore currently requires `mtls.mode: disabled` in the namespace `authbridge-runtime-config`; without it Envoy crash-loops on `Invalid path: /opt/svid_bundle.pem`. Use `proxy-sidecar` when mutual TLS between sidecars is required — its byte-peek listener implements `permissive` / `strict` modes today.
:::
