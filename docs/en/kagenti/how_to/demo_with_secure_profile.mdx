...
| `ImagePullBackOff` on an AuthBridge image | The `defaults.images` on the `Kagenti` operand points at a registry the cluster can't pull. Relocate the AuthBridge sidecar images (see [Enable the Secure Profile](./enable_secure_profile.mdx)) into a registry your cluster can reach. |
