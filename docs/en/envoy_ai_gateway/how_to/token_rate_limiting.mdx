...
<Steps>
  ...
  - For a password-protected Redis, also create an Opaque `Secret` with key `redis-username` and `redis-password`, then reference it via `rateLimit.backend.redis.auth.passwordRef`. See the [Envoy Gateway rate-limit docs](https://gateway.envoyproxy.io/docs/tasks/traffic/global-rate-limit/) for the full schema.
