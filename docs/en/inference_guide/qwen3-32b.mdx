...
| agg-mc-kv (warm) | 760 | 72.2 | 9922 | 7274 |
...
(TTFT 4612 → 760 ms, E2E 16.8 → 9.9 s, TPS +70%), where each turn shares a long
growing prefix. On scenario ② the **first (cold) run** still pays the full prefill of
the 16k base (TTFT ~3.5 s); the figures above are the warm steady state (runs 2-3).
Decode-step latency (ITL) is roughly the same between specs — it is not what the KV
store accelerates.
