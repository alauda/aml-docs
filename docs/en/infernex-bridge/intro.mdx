...
## Inference Engine Topologies

The inference engine topology is independent of the deployment entry point. Select the topology in the inference service or its referenced runtime configuration:

| Topology            | Behavior                                                                                                                                                    | Typical Use                                                                   |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| Aggregate           | The same engine workload handles prefill and decode. Mooncake and cache-indexer can still be enabled for KV cache reuse.                                    | A simpler deployment that does not require separate prefill and decode roles. |
| Prefill-decode (PD) | Prefill and decode run as separate workloads. Proxy-server coordinates the two roles, and Mooncake can transfer KV cache data between them when configured. | Independent scaling and tuning of the prefill and decode roles.               |
