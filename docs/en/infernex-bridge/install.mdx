...
target registry, for example:

```text
<target-registry>/3rdparty/openfuyao/ascend/vllm-ascend@sha256:...
```

Verify the imported image architecture and the runtime image ID:
...
kubectl get pod -n "${INFERENCE_NAMESPACE}" "${POD_NAME}" \
