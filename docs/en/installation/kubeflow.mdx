...
3. Ensure Alauda Build of KServe is installed
4. Deploy ASM in the business cluster where Kubeflow is to be deployed (if ASM was not deployed in the previous step) (Deploy with ASM v1 is deprecated, use ASM v2 instead, for more details about ASM v2 deployment, please refer to the ASM documentation)
5. Deploy LWS (Alauda Build of LeaderWorkerSet) plugin, which is a dependency of Kubeflow Trainer v2.
6. Configure the oauth2-proxy plugin as described below
...
In **Administrator** - **System Settings** - **Platform Parameters**, click the "Edit" button on the right of **Platform Access URLs**, and add the redirect URL, the format is `https://<your-kubeflow-domain>`, for example, `https://kubeflow.example.com`.
...
To confirm: In **Administrator** - **System Settings** - **Platform Parameters**, click the "Edit" button on the right of **Platform Access URLs**, and make sure there is an address in the format `https://<your-kubeflow-domain>`, where `<your-kubeflow-domain>` is the `kubeflowDomain` configured on the kfbase plugin deployment page.
...
#### Upgrading kfbase plugin (Kubeflow Base)

> **Note (UPGRADING)**: if you are upgrading from `v1.10.13` or older versions of kfbase plugin, the old versions of kfbase use `NodePort` to access the Kubeflow dashboard. After the upgrade, you should configure the DNS resolution or host file for `<your-kubeflow-domain>` to point to the IP address of the gateway assigned IP address, which is different from the old `NodePort` way of accessing. You can get the domain name in the `kubeflowDomain` field of the kfbase plugin configuration, and get the gateway assigned IP address by `kubectl -n istio-system get gateway kubeflow-external-gateway`.
...
> **Note (UPGRADING)**: if you are upgrading from `v1.10.9` or older versions of kfbase plugin, after the upgrade, you need to manually edit the parameter `pgStorageClass`, `pgStorageSize`, since these parameters are newly added in version `v1.10.10`.
