...
Deploy Kubeflow plugins in Alauda AI >= 1.3. Including:
...
- kfmr: kubeflow model registry instance (Helm Chart)
...
2. Ensure Alauda AI has been deployed (requires Alauda AI version >= 1.3)
3. Deploy ASM in the business cluster where Kubeflow is to be deployed (if ASM was not deployed in the previous step)
4. Configure the oauth2-proxy plugin as described below

### Configure oauth2-proxy Plugin

Refer to the steps in https://confluence.alauda.cn/pages/viewpage.action?pageId=213191378#heading-%E5%89%8D%E7%BD%AE%E4%BE%9D%E8%B5%96 to obtain the platform dex CA certificate.

Then go to the global cluster or in acp **Platform Management** -> **Resource Management** update the ServiceMesh resource, add the following content under the spec field (this method applies to servicemesh v1, for servicemesh v2 configuration method please consult relevant colleagues)
...
### 1. Deploy kfbase (KubeflowBase)

In *Cluster Plugins*, find the kfbase (KubeflowBase) plugin, fill in the configuration according to the page prompts, and wait for the component deployment to complete. After deployment, you need to perform the following operations to configure dex redirection:

Configure Global dex redirection. Log in to the Global cluster and execute: `kubectl -n cpaas-system edit configmap dex-configmap`, add configuration under `redirectURIs`, such as:

```yaml
      redirectURIs:
      - ...
      # Add the following line, note: the redirect address must be consistent with oidcRedirectURL in values:
...
### 4. Deploy kfp (KubeflowPipeline) and kftrainer (KubeflowTrainingOperator)

As above, in **Cluster Plugins**, find kfp (KubeflowPipeline) and kftrainer (KubeflowTrainingOperator).

**Note: After KubeflowPipeline deployment, Pipeline related functions can be used in the Kubeflow interface.**

**Note: KubeflowTrainingOperator is a background task scheduler and will not appear in the UI menu and functions.**

### 5. Deploy kfmr (KubeflowModelRegistry)

In **Catalog** or **Administrator** - **Marketplace** - **Chart Repositories**, find kfmr, click the "Create" button, fill in the deployment name, project, namespace (example deployment location), Chart Version, then copy the `values.yaml` configuration information from the right to the left, modify the following content according to the cluster information:
