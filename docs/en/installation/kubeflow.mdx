...
find the `dex-configmap` `ConfigMap` resource in the `cpaas-system` namespace, and click the
...
```yaml
      redirectURIs:
      - ...
      # Add the following line, replace the host and port with the actual values configured in oidcRedirectURL
      - https://192.168.139.133:30443/*
```

After deployment, you can find the Kubeflow menu item under the *Tools* navigation in AML. Click to enter the Kubeflow interface.
