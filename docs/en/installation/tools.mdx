...
When the MLflow Operator plugin is installed, it creates the MLflow Tools menu `ConfigMap` automatically from the operator configuration. Use the manual example below only for custom integrations or troubleshooting.
...
data:
  menu: |-
    - link: "https://<platform-access-address>/clusters/<cluster-name>/mlflow/"
      parentUid: advanced
      title: MLFlow
      titleI18N:
        en: MLFlow
