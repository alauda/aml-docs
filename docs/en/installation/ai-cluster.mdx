...
export PLATFORM_ADMINISTRATOR_USER=<administrator>  # [!code callout]
export PLATFORM_ADMINISTRATOR_PASSWORD=<administrator-password>  # [!code callout]
...
2. `${PLATFORM_ADMINISTRATOR_USER}` is the username of the ACP platform administrator.
3. `${PLATFORM_ADMINISTRATOR_PASSWORD}` is the password of the ACP platform administrator.
...
8. Select **Install Flavor** from dropdown:
...
12. Under **Gitlab** section:
    ...
    2.  Type `cpaas-system` fro **Administrator Token Secret Namespace**.
    3.  Type `aml-gitlab-administrator-token` for **Administrator Token Secret Name**.
