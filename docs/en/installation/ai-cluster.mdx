...
1. **Why does `aml-api-deploy` fail to update the database after install or upgrade?**

There is a known issue that the `aml-api-deploy` component may fail to initialize the database correctly after installation, and may also fail to migrate the database schema during upgrade. This can affect advanced features such as dataset preview and fine-tuning.

To fix this issue, enter the Pod of the `aml-api-deploy` Deployment in the `kubeflow` namespace:
