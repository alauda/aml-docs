...
The recommended target is Harbor. The following procedure has been verified with an HTTP Harbor registry, where the push command must include `--plain-http`.
...
export TAG=v2.3.0                             # [!code callout]
...
The tarball usually carries its own tag (e.g. `v2.3.0`) inside the OCI image layout. If needed, extract it from the tarball:
...
# 3. Push to Harbor. HTTP Harbor requires --plain-http.
