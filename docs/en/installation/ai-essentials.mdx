...
1. `${PKG_NAME}` is the name of the **Alauda AI Essentials** cluster plugin tarball.
2. `${PLATFORM_ADDRESS}` is your ACP platform address.
3. `${PLATFORM_ADMIN_USER}` is the username of the ACP platform admin.
4. `${PLATFORM_ADMIN_PASSWORD}` is the password of the ACP platform admin.
