...
**In-product branding**: This setting is applied on the cluster where Alauda AI is installed, not on the global cluster used by the resources above. Before running the command, switch your `kubectl` context to the Alauda AI installation cluster. To customize the product name, logo, and browser favicon displayed inside Alauda AI, update the `ui-config` `ConfigMap` in the `kubeflow` namespace. Set `data.override` to `"true"`, and then configure `data.productName`, `data.logo`, and `data.favicon`. The `logo` and `favicon` values must be complete data URI values, such as `data:image/svg+xml;base64,<base64-encoded-svg>` or `data:image/png;base64,<base64-encoded-png>`.
