...
Create the workbench (`PyTorch CANN` or `MindSpore CANN`) per [Creating a Workbench](../../develop/workbench/create_workbench.mdx) and upload the notebook through the JupyterLab file browser.
...
Either drop the model files there or change `HF_MODEL_DIR` in the first parameter cell. For versioned, reusable models, store the artifact in S3, a PVC, or an OCI registry. See [Upload Models Using a Workbench](../../deploy/model_management/upload_models_using_notebook.mdx); register the resulting URI separately in [Model Registry](../../deploy/model_management/model_repository.mdx#register-a-model-and-version).
...
Keep these on persistent storage. After training, upload the result to S3, a PVC, or an OCI registry with [Upload Models Using a Workbench](../../deploy/model_management/upload_models_using_notebook.mdx), then register its Model URI as a new Model Registry version using [Model Registry](../../deploy/model_management/model_repository.mdx#register-a-model-and-version).
