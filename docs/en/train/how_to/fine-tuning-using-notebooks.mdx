...
Create a workbench in **Alauda AI → Workbench** (or **Advanced → Kubeflow → Notebook**). The workbench itself should request only CPU — the GPU is requested by the VolcanoJob it submits. See [Creating a Workbench](../../develop/workbench/how_to/create_workbench.mdx).

## 2. Prepare the base model

Download `Qwen/Qwen3-0.6B` (or any HF model) and push it to the platform model repository. See [Upload Models Using Notebook](../../develop/how_to/upload_models_using_notebook.mdx).
