...
| Fine-tune (LlamaFactory CUDA) | NVIDIA GPU (CUDA 12.6) | LLaMA-Factory 0.9.4 + `git-lfs`, MLflow, MySQL/Postgres clients — runtime for [`fine-tune-with-trainer-v2.ipynb`](https://github.com/alauda/aml-docs/tree/master/docs/en/train/how_to/fine-tune-with-trainer-v2.ipynb) | `alaudadockerhub/fine_tune_with_llamafactory:v0.1.11` |
...
| Workbench PyTorch CANN | Huawei Ascend NPU (CANN 8.5) | Jupyter + PyTorch 2.9 + `torch_npu` 2.9 + MindSpeed-LLM — for the [Ascend NPU fine-tune / pretrain notebooks](./fine-tune-and-pretrain-llms-on-ascend-npu.mdx) and [`fine-tune-with-trainer-v2-mindspeed-npu.ipynb`](https://github.com/alauda/aml-docs/tree/master/docs/en/train/how_to/fine-tune-with-trainer-v2-mindspeed-npu.ipynb) | `alaudadockerhub/alauda-workbench-jupyter-pytorch-cann-py312-ubi9:v0.1.7` |
...
Ready-to-apply YAMLs live in [`assets/training-runtimes/`](https://github.com/alauda/aml-docs/tree/master/docs/en/train/how_to/assets/training-runtimes). Each YAML pins `:v0.1.0`; override the tag to track a different release. The YAMLs default to a Kubeflow `Profile` namespace — change `metadata.namespace` to the namespace where you submit jobs.

```bash
base=https://raw.githubusercontent.com/alauda/aml-docs/master/docs/en/train/how_to/assets/training-runtimes
# NVIDIA GPU
