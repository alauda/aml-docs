...
kubectl exec -naml-operator -it install/aml-operator -- /app/storageversion-migrate \
...
### Creating GitLab Administrator Token Secret

We need to create a secret for GitLab administrator token, which is required to run **Alauda AI Cluster**.
...
2. Create a MySQL administrator token secret named **aml-mysql-secret**.
...
spec:
  values:
    global:
      ...
      gitlabAdministratorTokenSecretRef:  # [!code callout]
        name: aml-gitlab-administrator-token
...
1. The `name` and `namespace` of GitLab administrator token secret created previously.
