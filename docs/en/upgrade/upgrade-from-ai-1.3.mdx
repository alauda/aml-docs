...
:::warning
The data migration you are about to perform is essentially an update to your inference services. For services running in **Knative Serverless mode**, this process involves a rolling update: a new `revision` will be created, and only once it's up and running will the old service be deleted.

Because of this, if your current inference service lacks sufficient resources (such as **GPU, CPU, or memory**) to complete a rolling update, please **stop** the service before executing the following script.
:::
