...
4. Verify the existing Workspaces and their PVCs are still present, then create and connect to a test Workbench. See [Migrate Existing Workspaces](#migrate-existing-workspaces) before declaring the migration complete.
...
## Migrate Existing Workspaces \{#migrate-existing-workspaces}
