...
1. Prepare model clients and modern wrappers (`llm_factory`, `embedding_factory`).
...
3. Iterate through rows and call `metric.ascore(...)` with metric-specific arguments.
   ...
   client = AsyncOpenAI(api_key="...", base_url="http://your-openai-compatible-endpoint/v1")
   llm = llm_factory("your-llm-model", client=client)
...
- **Credentials or endpoint configuration**: configure LLM API credentials (and an optional base URL for compatible gateways). If embeddings use a separate endpoint, configure embeddings credentials as well, then pass them to `llm_factory` / `embedding_factory`.
