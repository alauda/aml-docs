...
This guide shows how Kubeflow Pipelines (KFP) components log parameters, metrics, and models to [MLflow on Alauda AI](../components/mlflow/intro.mdx) with the **MLflow Python client**. Authentication and workspace/RBAC follow [Using the MLflow Python SDK with Authentication and RBAC](./mlflow-python-sdk.mdx) — each component authenticates with a user identity token and the server records the run under that user.
...
- Access to a KFP endpoint (see [Use Kubeflow Pipelines](../pipelines/kubeflow_pipelines.mdx)).
...
If you fine-tune with [Kubeflow Trainer v2](../../train/guides/fine-tune-with-trainer-v2.mdx), the framework's MLflow integration (for example `report_to: mlflow` in LLaMA-Factory) authenticates the same way. Trainer v2 uses `apiVersion: trainer.kubeflow.org/v1alpha1`, `kind: TrainJob`, and a `spec.runtimeRef` + `spec.trainer` shape. Point it at the in-cluster Service and inject the id token from a `Secret`:
...
See [Fine-tuning LLMs using Workbench](../../train/guides/fine-tuning-using-notebooks.mdx) for a full Trainer v2 + MLflow example.

For a longer solution that stitches this integration with the MLflow Model Registry, a TrustyAI `LMEvalJob`, and a daily KFP Recurring Run, see [Daily Fine-Tuning Pipeline with MLflow and TrustyAI](../../train/guides/fine-tuning-pipeline-with-mlflow-trustyai.mdx).
...
Proxied S3-compatible storage lets pipeline components upload through the tracking server without receiving S3 credentials. Configure `artifacts.s3` before logging artifacts or registering models (see [MLflow installation → High availability and storage](../components/mlflow/install.mdx#storage)).
...
| `403 PERMISSION_DENIED` | The token's user lacks access to the workspace namespace. Grant access to the MLflow workspace (see [MLflow Workspaces and Access Control](../../administer/multi_tenant/mlflow_workspaces.mdx)); no ServiceAccount is involved. |
