...
- Access to a KFP endpoint (see [Use Kubeflow Pipelines](../how_to/pipelines.mdx)).
...
If you fine-tune with [Kubeflow Trainer v2](../../train/how_to/fine-tune-with-trainer-v2.mdx), the framework's MLflow integration (for example `report_to: mlflow` in LLaMA-Factory) authenticates the same way. Trainer v2 uses `apiVersion: trainer.kubeflow.org/v1alpha1`, `kind: TrainJob`, and a `spec.runtimeRef` + `spec.trainer` shape. Point it at the in-cluster Service and inject the id token from a `Secret`:
...
See [Fine-tuning LLMs using Workbench](../../train/how_to/fine-tuning-using-notebooks.mdx) for a full Trainer v2 + MLflow example.

For a longer solution that stitches this integration with the MLflow Model Registry, a TrustyAI `LMEvalJob`, and a daily KFP Recurring Run, see [Daily Fine-Tuning Pipeline with MLflow and TrustyAI](../../train/how_to/fine-tuning-pipeline-with-mlflow-trustyai.mdx).
...
| `403 PERMISSION_DENIED` | The token's user lacks access to the workspace namespace. Grant access to the MLflow workspace (see [Workspaces and access control](../../administer/how_to/mlflow_workspaces.mdx)); no ServiceAccount is involved. |
