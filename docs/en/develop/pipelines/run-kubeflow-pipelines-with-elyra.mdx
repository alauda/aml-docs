...
Mount the PVC into an existing Workspace. The following snippet is a patch, not a
complete manifest: `spec.kind` and `spec.podTemplate.options` are required fields
on `Workspace`, so applying this YAML on its own is rejected. Patch the existing
object instead:
...
Replace `<workspace-name>`, `<your-namespace>`, and `<elyra-runtime-config-pvc>`
