...
- **`InferenceService`** (`serving.kserve.io/v1beta1`) — the standard KServe predictor used in [Create Inference Service using CLI](../../deploy/inference_service/how_to/create_inference_service_cli.mdx). Best for single-container model servers (vLLM, Triton, custom runtimes).
...
For the YAML fields and platform-specific labels/annotations the agent needs to reproduce, point it at [Create Inference Service using CLI](../../deploy/inference_service/how_to/create_inference_service_cli.mdx) as the canonical example. For exposing a new service externally, point it at [Configure External Access for Inference Services](../../deploy/inference_service/how_to/external_access_inference_service.mdx).
...
For model-size vs. GPU-memory selection, see the prior doc's [Choose a model that fits your hardware](./coding-agents-with-inference-service.mdx#best-practices) section. For autoscaling and cold-start trade-offs, see [Configure Scaling for Inference Services](../../deploy/inference_service/how_to/autoscale_settings.mdx). For interactive-latency wins, see [Speculative Decoding for vLLM Inference Services](../../deploy/inference_service/how_to/vllm_speculative_decoding.mdx).
...
|---|---|---|
| Interactive exploration, small dataset, one or two GPUs | Workbench Notebook | [Fine-tuning with Notebooks](../../train/how_to/fine-tuning-using-notebooks.mdx) |
| Production-grade SFT / OSFT with automatic memory management | Training Hub | [Fine-tuning LLMs with Training Hub](../../train/how_to/training-hub-fine-tuning.mdx) |
| Reusable templates, many runs, scheduled / batched on Kueue | Kubeflow Trainer v2 + LlamaFactory | [Fine-Tuning with Kubeflow Trainer v2](../../train/how_to/fine-tune-with-trainer-v2.mdx) |
...
- [Fine-Tuning with Kubeflow Trainer v2](../../train/how_to/fine-tune-with-trainer-v2.mdx)
- [Fine-tuning LLMs with Training Hub](../../train/how_to/training-hub-fine-tuning.mdx)
- [Fine-tuning with Notebooks](../../train/how_to/fine-tuning-using-notebooks.mdx)
