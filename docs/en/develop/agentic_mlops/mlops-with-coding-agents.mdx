...
- Configuring the inference traffic gateway — authentication and rate limits via [Alauda Build of Envoy AI Gateway](../../deploy/components/envoy_ai_gateway/intro.mdx).
...
For the YAML fields and platform-specific labels/annotations the agent needs to reproduce, point it at [Create Inference Service using CLI](../../deploy/inference_service/guides/create_inference_service_cli.mdx) as the canonical example. For exposing a new service externally, point it at [Configure External Access for Inference Services](../../deploy/inference_service/guides/external_access_inference_service.mdx).
...
For model-size vs. GPU-memory selection, see the prior doc's [Choose a model that fits your hardware](./coding-agents-with-inference-service.mdx#best-practices) section. For autoscaling and cold-start trade-offs, see [Configure Scaling for Inference Services](../../deploy/inference_service/guides/autoscale_settings.mdx). For interactive-latency wins, see [Speculative Decoding for vLLM Inference Services](../../deploy/inference_service/guides/vllm_speculative_decoding.mdx).
...
| ------------------------------------------------------------- | ---------------------------------- | ------------------------------------------------------------------------------------------- |
| Interactive exploration, small dataset, one or two GPUs       | Workbench Notebook                 | [Fine-tuning with Notebooks](../../train/guides/fine-tuning-using-notebooks.mdx)            |
| Production-grade SFT / OSFT with automatic memory management  | Training Hub                       | [Fine-tuning LLMs with Training Hub](../../train/guides/training-hub-fine-tuning.mdx)       |
| Reusable templates, many runs, scheduled / batched on Kueue   | Kubeflow Trainer v2 + LlamaFactory | [Fine-Tuning with Kubeflow Trainer v2](../../train/guides/fine-tune-with-trainer-v2.mdx)    |
| Already-tuned model needs to fit a smaller GPU before serving | LLM Compressor                     | [LLM Compressor with Alauda AI](../../deploy/model_compression/compressor_by_workbench.mdx) |
...
- Model artifact location (S3 URI / PVC URI / OCI image URI):
...
For experiment tracking and run metadata, [MLflow on Kubeflow](../components/mlflow/intro.mdx) is the platform-native option; tell the agent to log there from inside the training code so the report has a real source of truth.
...
- **Read-only first, write second.** Start every new task by asking the agent to read state (`get`, `describe`, logs, metrics) and _describe what it would do_ before making changes.
...
- [Fine-tuning with Notebooks](../../train/guides/fine-tuning-using-notebooks.mdx)
- [LLM Compressor with Alauda AI](../../deploy/model_compression/compressor_by_workbench.mdx)
- [MLflow on Kubeflow](../components/mlflow/intro.mdx)
