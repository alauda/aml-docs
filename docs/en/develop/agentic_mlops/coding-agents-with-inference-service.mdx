...
- A running, ready `InferenceService` that serves an OpenAI-compatible API. See [Create Inference Service using CLI](../../deploy/inference_service/how_to/create_inference_service_cli.mdx).
- Network access from the machine running the agent to the service endpoint. For access from a developer laptop outside the cluster, see [Configure External Access for Inference Services](../../deploy/inference_service/how_to/external_access_inference_service.mdx).
...
Deploy your model as an `InferenceService` following [Create Inference Service using CLI](../../deploy/inference_service/how_to/create_inference_service_cli.mdx), and if the agent runs outside the cluster, expose it following [Configure External Access for Inference Services](../../deploy/inference_service/how_to/external_access_inference_service.mdx).
...
:::tip
For reasoning-capable models (among the recommended set, GLM-5.2 and Qwen3.6, plus reasoning-mode Deepseek-v4 builds), also add the matching `--reasoning-parser` to the vLLM launch flags. See [Configure reasoning models and reasoning effort](#configure-reasoning-effort).
:::

## Step 2: Enable tool calling on the runtime \{#enable-tool-calling-on-the-runtime}

Coding agents work by calling tools (read file, write file, run shell). This requires the model to emit tool calls **and** vLLM to parse them. Add the following flags to the vLLM launch command in your `InferenceService` (in the sample from [Create Inference Service using CLI](../../deploy/inference_service/how_to/create_inference_service_cli.mdx), they go on the `python3 -m vllm.entrypoints.openai.api_server` line):
...
The table below shows parser pairs for reference. Parser names are version-specific and change across vLLM releases — always confirm against the [vLLM tool calling documentation](https://docs.vllm.ai/en/latest/features/tool_calling.html) for the version you run.

| Model family | `--tool-call-parser` | `--reasoning-parser` | Notes |
| --- | --- | --- | --- |
| Qwen3 / Qwen3.6 reasoning (`Qwen/Qwen3*`) | Check the current vLLM docs for the exact Qwen variant | `qwen3` | Reasoning is on by default; disable it with `chat_template_kwargs` if needed |
| GLM-5.2 (Zhipu AI) | Check the current vLLM docs for the GLM variant | Check the current vLLM docs for the GLM variant | Hybrid reasoning; the tool-call and reasoning parsers share the GLM family value |
| Deepseek-v4 (`deepseek_v4`) | Check the current vLLM docs for the DeepSeek variant | Check the current vLLM docs for the DeepSeek variant | Reasoning-mode builds only; see the [DeepSeek-V4-Flash (W4A8)](../../plan/validated_models/deepseek-v4-flash-w4a8.mdx) recipe |

The exact `--tool-call-parser` / `--reasoning-parser` values for these families depend on the vLLM version — read the model card and the [vLLM tool calling documentation](https://docs.vllm.ai/en/latest/features/tool_calling.html) before committing, and verify tool calls round-trip end-to-end.
...
### Recommended models for coding agents \{#recommended-models}

As a best practice, build on-premise coding agents on the following five model families — and, to keep results predictable, treat them as the shortlist rather than reaching for other models. Each one combines strong code generation, reliable instruction following, and native tool/function calling, and each ships in a range of sizes and quantization formats so you can fit it to the accelerator you have. Pair every choice with a hardware-matched quantization: an [Unsloth](https://huggingface.co/unsloth) GGUF build on GPU, or a **ModelSlim (`msModelSlim`)** W8A8/W4A8 build on Ascend NPU (see [Quantized variants to fit your hardware](#quantized-variants)).

| Model | Type | Why it works for coding agents | Fit & notes |
| --- | --- | --- | --- |
| **GLM-5.2** (Zhipu AI) | Large MoE | Frontier agentic reasoning and multi-step tool use; excels at long, planned edits that span many files. | Heaviest of the set — serve quantized (GGUF / ModelSlim) or tensor-parallel across multiple accelerators. |
| **Qwen3.6-27B-MTP** (Qwen) | Dense `qwen3_5` hybrid + MTP | Strong general-purpose coding; the native multi-token-prediction head speeds up decode for snappier interactive turns. | W8A8 validated on Ascend → [Qwen3.6-27B (W8A8)](../../plan/validated_models/qwen3-6-27b-w8a8.mdx). ~33–36 GB quantized. |
| **Qwen3.6-35B-A3B** (Qwen) | MoE, ~3B active | Best quality-per-VRAM in the set: activates only ~3B parameters per token while carrying a 35B knowledge base. | Runs comfortably on a single 24 GB GPU — the recommended default for most users. |
| **Gemma4-12B-it / Gemma4-31B-it** (Google) | Dense, instruction-tuned | Clean instruction following and dependable tool calls. 12B fits mid-range GPUs; 31B raises quality on 40–48 GB cards. | Pick **12B** for latency, **31B** for quality. |
| **Deepseek-v4** (DeepSeek) | Large MoE (MLA + DSA + MTP) | Frontier-grade coding and agentic planning; the sparse MoE keeps active parameters low despite a large expert pool. | Heavy — serve quantized or multi-GPU/-card. W4A8 validated on Ascend → [DeepSeek-V4-Flash (W4A8)](../../plan/validated_models/deepseek-v4-flash-w4a8.mdx). |

Confirm the matching vLLM `--tool-call-parser` (and `--reasoning-parser` for the reasoning-capable families) against the vLLM version you run — see [Enable tool calling on the runtime](#enable-tool-calling-on-the-runtime).
...
#### Quantized variants to fit your hardware \{#quantized-variants}
...
**GPU — Unsloth GGUF.** [Unsloth](https://huggingface.co/unsloth) publishes GGUF builds of the latest models, optimized for fast loading with vLLM (`--quantization gguf`). Its QAT (quantization-aware training) builds hold quality at aggressive bit-widths better than post-hoc quantization. The most useful builds for the recommended models:

| Model | Format | Active params | VRAM (approx.) | Notes |
| --- | --- | --- | --- | --- |
| [`unsloth/gemma-4-12b-it-GGUF`](https://huggingface.co/unsloth/gemma-4-12b-it-GGUF) | GGUF | 12B | ~16 GB | Fits a single mid-range GPU |
| [`unsloth/Qwen3.6-35B-A3B-GGUF`](https://huggingface.co/unsloth/Qwen3.6-35B-A3B-GGUF) | GGUF (MoE) | ~3B active | ~12 GB | Best quality/VRAM ratio; fits 24 GB |
...
| [`unsloth/gemma-4-31b-it-GGUF`](https://huggingface.co/unsloth/gemma-4-31b-it-GGUF) | GGUF | 31B | ~40 GB | Highest-quality dense Gemma 4 |
| `unsloth/GLM-5.2-GGUF` | GGUF (MoE) | large | multi-GPU | Frontier agentic; shard with `--tensor-parallel-size` |
| `unsloth/Deepseek-v4-GGUF` | GGUF (MoE) | large | multi-GPU | Frontier coding; shard across GPUs |

> **Note:** Unsloth repo names follow the `unsloth/<model>-GGUF` convention — confirm the exact path on the [Unsloth org page](https://huggingface.co/unsloth) before pulling, especially for the large `GLM-5.2` and `Deepseek-v4` builds, which are usually split into multiple files. For AWQ or GPTQ INT4 variants instead, search [huggingface.co/models](https://huggingface.co/models?sort=trending&search=AWQ) for e.g. `qwen3.6 AWQ` or `gemma-4 GPTQ`.
...
| Model | ModelSlim format | Topology | Recipe |
| --- | --- | --- | --- |
| Qwen3.6-27B | W8A8 (~33 GB) | Ascend 910B4 ×8 (2 × TP=4) | [Qwen3.6-27B (W8A8)](../../plan/validated_models/qwen3-6-27b-w8a8.mdx) |
| DeepSeek-V4-Flash | W4A8 (~151 GB) | Ascend 910B4 ×8 (1 × TP=8) | [DeepSeek-V4-Flash (W4A8)](../../plan/validated_models/deepseek-v4-flash-w4a8.mdx) |

To quantize your own weights on NPU, use the ModelSlim workbench image and its verification notebook (see [Create a Workbench](../workbench/how_to/create_workbench.mdx)).

#### Hardware fit guide

| Accelerator memory | Example hardware | Recommended model |
| --- | --- | --- |
| 12–16 GB | L4, A10, RTX 4070 | `Qwen3.6-35B-A3B` (MoE, ~3B active) or `Gemma4-12B-it` |
| 24 GB | A30 (24G), RTX 4090 | `Qwen3.6-35B-A3B` (MoE) — the recommended default |
| 40–48 GB | A40, L40S, A6000 | `Qwen3.6-27B-MTP` or `Gemma4-31B-it` (GGUF) |
| 80 GB | A100-80G, H100, H800 | `Qwen3.6-27B-MTP` or `Gemma4-31B-it` at FP16 |
| Ascend NPU (8 × 32 GB) | Ascend 910B4 ×8 | `Qwen3.6-27B` (W8A8) or `Deepseek-v4` (W4A8), ModelSlim — see [Inference Guide](../../plan/validated_models/index.mdx) |
| Multi-GPU / multi-card (2–8×) | 2–8 × 80 GB | `GLM-5.2` or `Deepseek-v4` (large MoE, tensor-parallel) |
...
- **Confirm a matching vLLM parser exists** for your chosen model (see [Enable tool calling on the runtime](#enable-tool-calling-on-the-runtime)) before committing to it. Verify the `--tool-call-parser` (and `--reasoning-parser` for the reasoning-capable GLM-5.2, Qwen3.6, and Deepseek-v4) in the vLLM docs for your version.
...
- **Enable chunked prefill** (`--enable-chunked-prefill`) when long prompts cause latency spikes under concurrency, so decode steps are not starved by a large prefill. Note the [CLI sample](../../deploy/inference_service/how_to/create_inference_service_cli.mdx) disables it by default.
...
- **Enable MTP / speculative decoding** for lower interactive latency on agent loops — the recommended `Qwen3.6-27B-MTP` and `Deepseek-v4` ship a native multi-token-prediction head. See [Enable MTP (multi-token prediction)](#enable-mtp) below and [Speculative Decoding for vLLM Inference Services](../../deploy/inference_service/how_to/vllm_speculative_decoding.mdx).
- **Mind autoscaling and cold starts.** For interactive single-user agent use, keep `minReplicas: 1` — scaling from zero adds a multi-minute cold start that is painful mid-task. For bursty multi-developer usage, configure autoscaling deliberately; see [Configure Scaling for Inference Services](../../deploy/inference_service/how_to/autoscale_settings.mdx) and [Set Up Autoscaling for Inference Services with KEDA](../../deploy/inference_service/how_to/keda_autoscaling.mdx).
- **Allow long requests.** Agent turns can be long-running; size the Knative `serving.knative.dev/progress-deadline` annotation and your client timeouts accordingly. If requests are cut off, see [Inference timeout troubleshooting](../../deploy/inference_service/trouble_shooting/infer_timeout.mdx).

#### Enable MTP (multi-token prediction) \{#enable-mtp}
...
Confirm the exact `method` against your vLLM version, and treat the full recipe — including the MTP guardrails these need on Ascend (disabled prefix caching, a capped `--max-num-seqs`, `enforce_eager`) — as the source of truth: [Qwen3.6-27B (W8A8)](../../plan/validated_models/qwen3-6-27b-w8a8.mdx) and [DeepSeek-V4-Flash (W4A8)](../../plan/validated_models/deepseek-v4-flash-w4a8.mdx). For other speculative methods (EAGLE-3, N-gram) and end-to-end verification and rollback, see [Speculative Decoding for vLLM Inference Services](../../deploy/inference_service/how_to/vllm_speculative_decoding.mdx).
...
- **Requests time out on long tasks.** Increase the Knative `progress-deadline` annotation and the client timeout — see [Inference timeout troubleshooting](../../deploy/inference_service/trouble_shooting/infer_timeout.mdx).
...
- [Create Inference Service using CLI](../../deploy/inference_service/how_to/create_inference_service_cli.mdx)
- [Configure External Access for Inference Services](../../deploy/inference_service/how_to/external_access_inference_service.mdx)
- [Configure Scaling for Inference Services](../../deploy/inference_service/how_to/autoscale_settings.mdx)
- [Set Up Autoscaling for Inference Services with KEDA](../../deploy/inference_service/how_to/keda_autoscaling.mdx)
- [Speculative Decoding for vLLM Inference Services](../../deploy/inference_service/how_to/vllm_speculative_decoding.mdx)
- [Extend Inference Runtimes](../../deploy/inference_service/how_to/custom_inference_runtime.mdx)
