...
The complete Ray offline-store configuration, KubeRay deployment, production-scale guidance, and smoke test are provided as the [Feast with Ray offline store notebook](https://github.com/alauda/aml-docs/tree/master/docs/en/train/guides/feast-with-ray-offline-store.ipynb). Keep the notebook and its adjacent `assets/feast-with-ray-offline-store/` directory together when downloading it.
