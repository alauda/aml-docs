...
Workspace membership and permissions are therefore managed with ordinary Kubernetes `Role` / `RoleBinding` objects — see [MLflow Workspaces and Access Control](../../../administer/multi_tenant/mlflow_workspaces.mdx).

## When to use it

Use the MLflow Operator when teams need a shared, SSO-protected experiment-tracking, [AI-agent tracing](../../experiment_tracking/agent-tracing.mdx), and model-registry service where each team's runs and models are isolated by namespace and governed by Kubernetes RBAC — rather than an unauthenticated, single-tenant MLflow server.
