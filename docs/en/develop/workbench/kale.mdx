...
Set it to a dedicated runtime image when the pipeline needs packages or system
libraries that are not part of the Workbench image. The image must be pullable by
the KFP worker nodes (and by both architectures when the cluster is multi-arch),
not only by the JupyterLab pod.
