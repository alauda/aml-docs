...
For the underlying model, see [Introduction → Multi-tenancy model](../../develop/components/mlflow/intro.mdx#multi-tenancy).
...
:::info
The tracking server's **default workspace** namespace must exist and be labelled **before** the server first starts, or it will not boot (see [Installation → Prerequisites](../../develop/components/mlflow/install.mdx)).
:::
...
You can only use a workspace your account has access to. For full authentication details — obtaining an identity token browser-free and connecting the SDK — see [Using the MLflow Python SDK with Authentication and RBAC](../../develop/experiment_tracking/mlflow-python-sdk.mdx).
