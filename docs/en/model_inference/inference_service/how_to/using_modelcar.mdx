...
- Administrator access to enable Modelcar in AmlCluster

## Enabling Modelcar in Alauda AI Platform
...
- Modelcar must be enabled in AmlCluster as described earlier
...
2. `aml.cpaas.io/runtime-type: vllm` specifies the code runtime type. For more information about custom inference runtimes, see [Extend Inference Runtimes](./external_access_inference_service.mdx).
...
3. **Modelcar Not Enabled**: Check that Modelcar is properly enabled in the AmlCluster configuration

### Debugging Steps

1. Check the InferenceService events:
   ```bash
   kubectl describe inferenceservice oci-demo -n demo-space
   ```

2. Check the predictor pod logs:
   ```bash
   kubectl logs -n demo-space -l serving.kserve.io/inferenceservice=oci-demo
   ```

3. Verify the model image can be pulled:
   ```bash
