...
Follow the instructions in [Create Workbench](../../../workbench/how_to/create_workbench.mdx) to create a new Workbench instance. When creating the Workbench, make sure to select the image `odh-workbench-jupyter-pytorch-llmcompressor-cuda-py312-ubi9`, which includes the required LLM Compressor environment. Model compression is currently supported only within **JupyterLab**.
...
Once compression is complete, upload the compressed model back to the model repository. See [Upload Models Using Notebook](../../model_management/how_to/upload_models_using_notebook.mdx) for detailed steps on uploading model files to the model repository.
