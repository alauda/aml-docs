...
- [Authenticating Consumers](./how_to/identity_authentication) — verify SSO tokens or API keys and propagate caller identity.
- [Configuring Token Quotas](./how_to/token_rate_limiting) — enforce per-user, per-department, and per-tier token budgets.
- [Metering Token Usage](./how_to/usage_metering) — report consumption per tenant and feed chargeback.
- [Routing to LLM Providers](./how_to/external_provider_routing) — front external providers with credential injection and failover.
- [Charging Back Token Usage](./how_to/cost_management_chargeback) — price token usage into per-namespace bills with Cost Management.
