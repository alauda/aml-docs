...
This page covers ingesting reference data via `POST /data/upload` and live inference via `POST /consumer/kserve/v2`, registering drift and bias metrics through the HTTP API, and exposing time series to Prometheus for monitoring dashboards.

## Prerequisites

- A `TrustyAIService` in `Ready` state in the namespace of the model you want to monitor. See [Deploy TrustyAI Service](../evaluate_safety/components/trustyai/deploy_tas.mdx).
